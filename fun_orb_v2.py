# -*- coding: utf-8 -*-
"""
Created on Tue Oct 19 12:18:42 2021

@author: User
"""

"""here we trying to add a pixrels with mean background signals around the object 
(instead of using background itself)"""
...
def orb_watching(frame, imgs0, xy_imgs0, xy_objs, delt_xy, edge_width, test_mode=0):
    ...
    for i_obj, img0 in enumerate(imgs0):
        ...
        img1_src = frame[xy_im1_roi[1] : xy_im1_roi[3], xy_im1_roi[0] : xy_im1_roi[2]]
        img1_smooth = cv2.blur(img1_src, (n_kern, n_kern))
        ...
        if isshow_kp == 1:
            ...
            from my_standart_modules import showing_img
            showing_img(img0, im_sc=5)
            showing_img(img1, im_sc=5)
        ...
        if (kp0 != []) & (kp1 != []): #there was found some keypoints in sample and in a current frame fragment
            ...
            if matches:        
                ...
                matches_good = []
                # !!!!!!!!!!!!here we use a small threshold because of the respectivly clear background
                thr_good = (1 - 0.1)*(matches[len(matches)-1].distance -  matches[0].distance) + matches[0].distance
            ...
            if test_mode == 1:
                ...
                img_match_frame = cv2.drawMatches(img0, kp0, frame, kp1_frame, matches[:im_show_matches], None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
