...
"""version of fun_orb_v4
   Using a labeling information for object coordinates (instead of the keypoints info)
        ...
        if (kp0 != []) & (kp1 != []): #there was found some keypoints in sample and in a current frame fragment
            ...
            if matches:        
                ...
                if matches_good:
                    ...
                    thrsh_lbl = 0.7 #!!!!!magical number (threshold for labeling)
                    lbl_data = ski_msr.label(img1 > thrsh_lbl * np.max(img1), connectivity=2)
                    show_gr(img1, im_sc = 3)
                    show_gr(img1 > thrsh_lbl * np.max(img1), im_sc = 3) #!!!!!
